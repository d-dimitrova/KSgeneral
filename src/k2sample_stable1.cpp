...
double kuiperks(int nx, int ny, int M[], int lengthM, double dstatup, double dstatdown , double tol){
// Based on ALGORITHM AS 288 APPL.STATIST. (1994), VOL.43, NO.1
...
double kuiper2sample_cpp(int nx, int ny, int M[], int lengthM, double q, double tol)
{
...
// Calculate the least common multiple of nx and ny
	nxy = nx * ny / gcd(nx, ny);
	nxydouble = nx * ny / (double)gcd(nx, ny);
//	lfacto = lgamma(nx+1) + lgamma(ny+1)- lgamma(nx + ny +1);
	...
	if(C <= 0)
	{
		pval = kuiperks(nx, ny, M, lengthM, (double)(0.0), (double)(0.0), eps);
		if(pval < -1.5) return 1.0;
		pval =  1 - exp(pval + lfacto);
		return pval;
	}
	else if((C > 0) && (C <= nxy - 1))
	{
		...
		pval_vec_pos[C] = kuiperks(nx, ny, M, lengthM, C / nxydouble, (double)(0.0), eps);
		if(pval_vec_pos[C] < -2.5) return -3.0;
        minmum = pval_vec_pos[C];
		for(i = 0; i <= C-1; i++)
		{
// Find  the number of trajectories with specific bound of Kolmogorov-Smirnov statistic
			pval_vec_pos[i] = kuiperks(nx, ny, M, lengthM, i / nxydouble, (C - i) / nxydouble , eps);
			pval_vec_neg[i] = kuiperks(nx, ny, M, lengthM, i / nxydouble, (C - i - 1) / nxydouble , eps);
...
// Check whether there are overflows in calculation
	...
	else
	{
		...
		pval_vec_pos[p] = kuiperks(nx, ny, M, lengthM, double(1.0), (C - nxy) / nxydouble , eps);
		if(pval_vec_pos[p] < -2.5) return -3.0;
        minmum = pval_vec_pos[p];
		for(i = 0; i <= p-1; i++)
		{
			pval_vec_pos[i] = kuiperks(nx, ny, M, lengthM, (C - nxy + i) / nxydouble, (nxy - i) / nxydouble , eps);
			pval_vec_neg[i] = kuiperks(nx, ny, M, lengthM, (C - nxy + i) / nxydouble, (nxy - i - 1) / nxydouble , eps);
			if((pval_vec_pos[i] < -2.5) || (pval_vec_neg[i] < -2.5)) return -3.0;
...
double Kuiper2sample_Rcpp(int m, int n, Rcpp::IntegerVector M, double q, double tol){
    const int lengthM=M.size();
    double pval = 0;
    pval = kuiper2sample_cpp(m,n,M.begin(),lengthM,q,tol);
    return(pval);
}
